// Reinterleave: separate r[] and s[] → [r0,s0,r1,s1,...].
...
@group(0) @binding(2) var<storage, read_write> interleaved: array<f32>; // [n_pos * n_bands * 2]
@group(0) @binding(3) var<uniform> params: Params;

@compute @workgroup_size(64)
fn reinterleave(@builtin(global_invocation_id) gid: vec3<u32>) {
    let flat = gid.x;
    ...
    interleaved[interleaved_base + band * 2u] = r_in[flat];
    interleaved[interleaved_base + band * 2u + 1u] = s_in[flat];
}
