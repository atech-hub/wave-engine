//! Legacy backward ops — ODE backward + attention backward.
//!
//! Pure math primitives (linear, layer_norm, gelu, softplus backward)
//! are now in common/backward.rs. Re-exported here for API compatibility.

use crate::model::*;

// ─── Re-exports from common ────────────────────────────────────

pub use crate::common::backward::{
    linear_backward, linear_backward_dx_only, linear_no_bias_backward,
    layer_norm_backward, gelu_backward, softplus_backward,
};
...
pub fn kerr_ode_backward(
    ...
    for _ in 0..n_steps {
        let (r_new, s_new) = crate::model::rk4_step_public(
            &r, &s, dt, &gamma, &weights.omega, weights.alpha, weights.beta, weights.chi, &weights.rk4_weights,
        );
        r = r_new;
        s = s_new;
        states.push((r.clone(), s.clone()));
