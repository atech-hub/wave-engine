...
impl ComputeBackend for GpuBackend {
    ...
    fn linear_backward_dx_batch(&self, d_y: &[Vec<f32>], w: &[Vec<f32>]) -> Vec<Vec<f32>> {
        ...
        let wt_buf = self.storage_buf("mvbb_wt", &wt_flat);
        let dy_buf = self.storage_buf("mvbb_dy", &dy_flat);
        let dummy_bias = [0.0f32];
        let db_buf = self.storage_buf("mvbb_db", &dummy_bias);
        ...
        let mut encoder = self.device.create_command_encoder(&Default::default());
        { let mut pass = encoder.begin_compute_pass(&Default::default());
          pass.set_pipeline(&self.matvec_batch_pipeline); // SAME shader as forward!
          pass.set_bind_group(0, &bind_group, &[]);
          pass.dispatch_workgroups(in_dim as u32, n_pos as u32, 1); }
        self.queue.submit(Some(encoder.finish()));

        let dx_flat = pool.readback_scratch(&self.device, &self.queue, 0, out_total);
        drop(pool);
